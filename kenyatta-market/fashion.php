...
// Fetch fashion items
if ($searchTerm) {
  $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'fashion' AND title LIKE ?");
  $likeTerm = '%' . $searchTerm . '%';
  $stmt->bind_param("s", $likeTerm);
} else {
  $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'fashion'");
}
$stmt->execute();
...
<head>
  <meta charset="UTF-8">
  <title>Fashion & Style - Kenyatta Market</title>
...
<div class="bg-red-600 text-white py-16">
  <div class="max-w-7xl mx-auto px-4 text-center">
    <h1 class="text-4xl font-bold mb-4">Fashion & Style</h1>
    <p class="text-xl">Trendy clothing and accessories for every occasion</p>
  </div>
</div>
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
      <div class="bg-white rounded-lg overflow-hidden shadow group cursor-pointer">
        ...
        <div class="p-4">
          ...
          <p class="text-2xl font-bold text-red-600 mb-2">KSh <?= number_format($row['price']) ?></p>
          ...
          <form method="POST" action="cart.php">
            ...
            <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded flex items-center justify-center">
              🛒 Add to Cart
            </button>
          </form>
        </div>
