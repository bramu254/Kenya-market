...
<!-- Alert messages -->
<?php if ($success): ?>
  <div class="bg-green-500 text-white px-4 py-3 text-center"><?= $success ?></div>
<?php elseif ($error): ?>
  <div class="bg-red-500 text-white px-4 py-3 text-center"><?= $error ?></div>
<?php endif; ?>
        ...
        <div class="p-4">
          ...
          <?php if ((int)$product['stock'] > 0): ?>
            <form method="POST" action="add-to-cart.php" class="space-y-2">
              ...
              <input type="number" name="quantity" min="1" max="<?= $product['stock'] ?>" value="1" class="w-full border rounded px-2 py-1 text-sm">
              <button type="submit" class="w-full h-10 bg-green-600 hover:bg-green-700 text-white rounded">Add to Cart</button>
            </form>
          <?php else: ?>
            <div class="text-red-600 font-semibold text-center">Out of Stock</div>
          <?php endif; ?>
        </div>
      </div>
    <?php endforeach; ?>
