...
// Fetch Health & Beauty items
if ($searchTerm) {
  $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'health-beauty' AND title LIKE ?");
  $likeTerm = '%' . $searchTerm . '%';
  $stmt->bind_param("s", $likeTerm);
} else {
  $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'health-beauty'");
}
$stmt->execute();
...
<head>
  <meta charset="UTF-8">
  <title>Health & Beauty - Kenyatta Market</title>
...
<div class="bg-purple-600 text-white py-16">
  <div class="max-w-7xl mx-auto px-4 text-center">
    <h1 class="text-4xl font-bold mb-4">Health & Beauty</h1>
    <p class="text-xl">Wellness products and beauty essentials</p>
  </div>
</div>
    ...
    <?php while ($row = $result->fetch_assoc()): ?>
      <div class="bg-white rounded-lg overflow-hidden shadow group cursor-pointer">
        ...
        <div class="p-4">
          ...
          <p class="text-2xl font-bold text-purple-600 mb-2">KSh <?= number_format($row['price']) ?></p>
          ...
          <form method="POST" action="cart.php">
            ...
            <button type="submit" class="w-full bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded flex items-center justify-center">
              🛒 Add to Cart
            </button>
          </form>
        </div>
